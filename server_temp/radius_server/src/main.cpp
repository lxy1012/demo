...
int main(int argc, char *argv[])
{
	...
	conf.level = _ERROR; //初始化error

	strcpy(conf.log_file_name, "../log/radius");
	init_log_conf(conf);	

	//写版本号
	_log(_DEBUG, "radius version", "v1.01-----20220311");
	...
	CMySQLConnection hMySQLConnectionUser1;
	hMySQLConnectionUser1.SetConnectionConfig(stMySQLConfig);
	hMySQLConnectionUser1.SetConnectionName("gcuser1");
	bool bConnectOK = hMySQLConnectionUser1.OpenConnection();
	if (!bConnectOK)
	{
		_log(_ERROR, "DB", "cant't connect to gcuser1:host[%s:%d],[%s]:[%s]", stSysConfigBaseInfo.szDBGameHost, stSysConfigBaseInfo.iDBGamePort, stSysConfigBaseInfo.szDBGameDataBase, stSysConfigBaseInfo.szDBGame);
		exit(0);
	}

	CMySQLConnection hMySQLConnectionUser2;
	hMySQLConnectionUser2.SetConnectionConfig(stMySQLConfig);
	hMySQLConnectionUser2.SetConnectionName("gcuser2");
	bConnectOK = hMySQLConnectionUser2.OpenConnection();
	if (!bConnectOK)
	{
		_log(_ERROR, "DB", "cant't connect to gcuser2:host[%s:%d],[%s]:[%s]", stSysConfigBaseInfo.szDBGameHost, stSysConfigBaseInfo.iDBGamePort, stSysConfigBaseInfo.szDBGameDataBase, stSysConfigBaseInfo.szDBGame);
		exit(0);
	}

	MsgQueue *pGetQueue = new MsgQueue(2000, 1024 * 7 + 512);
	MsgQueue *pSendQueue = new MsgQueue(2000, 1024 * 7 + 512);
	MsgQueue *pInnerQueue = new MsgQueue(2000, 1024 * 7 + 512);
	...
	ServerLogicThread *pLogicThread = new ServerLogicThread(&hMySQLConnectionUser1);
	pLogicThread->Initialize(pGetQueue, pSendQueue, pInnerQueue);
	pLogicThread->Start();		

	//处理内部消息的线程
	Dispatcher *pDispathThread = new Dispatcher();
	pDispathThread->Initialize(pInnerQueue, &hMySQLConnectionUser2);
	pDispathThread->Start();

	_log(_ERROR, "main", "v1.01 radius run ok---serverport[%d]", stSysConfigBaseInfo.iServerPort);

	sem_wait(&ProcBlock);		//在此永远阻塞主线程

	hMySQLConnectionUser1.CloseConnection();	//退出数据库
	hMySQLConnectionUser2.CloseConnection();	//退出数据库
