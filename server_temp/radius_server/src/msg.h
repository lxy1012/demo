...
#define GAME_CHECK_FRIENDSHIP_MSG			0x20A	//查询用户是否存在好友关系
...
typedef struct GameServerIPInfo
{
	...
	unsigned short sServerPort;//服务器端口
	uint iServerIPBak;//服务器ip备份
	unsigned short sServerPortBak;//服务器端口备份
}GameServerIPInfoDef;
...
typedef struct GameCheckFriendshipReq		//GAME_CHECK_FRIENDSHIP_MSG 检查iUserID与iCheckID中几个玩家是否有好友关系
{
	MsgHeadDef msgHeadInfo;
	int iUserID;
	int iCheckID[9];
}GameCheckFriendshipReqDef;

typedef struct GameCheckFriendshipRes	  //GAME_CHECK_FRIENDSHIP_MSG
{
	MsgHeadDef msgHeadInfo;
	int iUserID;
	char cExist;   //是否存在好友关系
}GameCheckFriendshipResDef;
...
typedef struct GameGetFriendListReq		//GAME_GET_FRIEND_LIST_MSG 获取好友列表
{
	MsgHeadDef msgHeadInfo;
	int iUserID;
}GameGetFriendListReqDef;

typedef struct GameGetFriendListRes	  //GAME_GET_FRIEND_LIST_MSG
{
	MsgHeadDef msgHeadInfo;
	int iUserID;
	int iFriendsNum;	//后面跟iFriendsNum个iFriendID
	//iFriendID[];
}GameGetFriendListResDef;
...
typedef struct GameUserAuthenInfoFailRes
{
	...
	int	iLastServerID;//所卡房间serverid
