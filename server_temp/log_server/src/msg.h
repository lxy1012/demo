...
#define GAME_ACHIEVE_TASK_LOG_MSG			0x263	//成就任务日志
...
typedef struct GameUserAccountGame
{
	...
	int	iAllNum;//总游戏增加局数
	long long	iWinMoney;//总赢游戏币（不包括桌费，仅指游戏输赢）
	long long	iLoseMoney;//总输游戏币（不包括桌费，仅指游戏输赢）
	long long	iTableMoney;//总桌费（计费时不缩小）
	int	iGameAmount;//游戏总净分（计费时不缩小）
...
typedef struct GameSpecialAccontMsg//GAME_SPECIAL_ACCOUNT_MSG 游戏中对主要货币特殊计费
{
	...
	int	iServerID;
	char szUserToken[32];
	int	iMoneyType;//iMoneyType=0游戏币 1钻石 2奖券
	long long	iAddNum;
	long long iNowNum;
