...
public:
    ...
    //added by sff for update number 2022/03/14(update时可以对数据进行增加操作)
    void AddFieldValueNumber(const std::string& strField, int64 nValue, bool bPrimaryKey = false);
	//added by zwr for update number 2023/06/05(update时可以对数据进行减操作)
    ...
    void AddQueryConditionStr(const std::string& strFieldName, const std::string& strValue, ERelationType nRelationType = CSQLWrapper::ERelationType::Equal);//nRelationType为NumIn/StirngIn时,strValue为用&拼接的字符串，目前仅用于select语句
