...
{
	MsgHeadDef msgHeadInfo;
	int iReqType;//请求类型（0游戏登录room服务器，1中心服务器登录room服务器,2游戏服务器登录中心以及其他底层服务器,3公告服务器登录room/redis服务器）
	int iServerID;
...
typedef struct GameOneRoomOnline
{
	...
	int iOnlinePC;//该类型房间PC在线人数(改为微信小游戏人数)
...
const int g_iSeverIDSysError = 20;//计费时发现和当前serverid不同步
...
typedef struct GameUserAccountGame
{
	...
	int	iAllNum;//总游戏增加局数
	int	iWinMoney;//总赢游戏币（数据库内缩小1000倍，计费时不缩小，不包括桌费，仅指游戏输赢）
	int	iLoseMoney;//总输游戏币（数据库内缩小1000倍，计费时不缩小，不包括桌费，仅指游戏输赢）
	int	iTableMoney;//总桌费（数据库内缩小1000倍，计费时不缩小）
	int	iGameAmount;//游戏总净分（数据库内缩小1000倍，计费时不缩小）
...
typedef struct ServerLogLvConfResForBull	//SERVER_LOG_LEVEL_SYNC_MSG 给公告服务器同步日志等级信息（额外需要所连接的redis和room服务器信息）
{
	MsgHeadDef msgHeadInfo;
	int iLogLevel;
	uint iServerIP;//自己的ip 
	int iBaseServerNum;// 需要连接的底层服务器数量(公告服务器需要连接主redis和room服务器)，后接iBaseServerNum个GameServerIPInfo
} ServerLogLvConfResForBullDef;
...
typedef struct GameLogOnePlayerLog
{
	...
	int	iWinMoney;//本局赢游戏币(不包括桌费)
	int	iLoseMoeny;//本局输游戏币(不包括桌费)
	int	iExtraAddMoney;//本局内非游戏获得的游戏币（如充值，抽奖等）
	int	iExtraLoseMoney;//本局内非游戏减少的游戏币(如使用道具扣除等)
	char cExtraMoneyDetail[160];//非游戏币计费详情(计费类型1_计费数目1&计费类型2_计费数目2...)
	int	iPlayType;//本局玩法
	int	iTableMoney;//本局桌费
...
typedef struct AuthenReq	//AUTHEN_REQ_MSG  玩家登陆验证请求
{
	...
	char cLoginType;	//平台类型0pc,1android,2iphone.3ipad
	char cExtraFlag1;
	char cExtraFlag2;
...
{
	MsgHeadDef msgHeadInfo;
	int iUsers[5];//可同桌游戏的玩家（目前最多只有4个，预留一个）
	int iUserServerID[5];//这些玩家所在游戏服务器
	int iIfFreeRoom[5];//这些玩家所在房间类型
